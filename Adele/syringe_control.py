...
class Syringe(object):
    ...
    def __init__(self, identifier, syringeVolume, syringe_type=None):
        ...
        if self.syringe_type == 'bdp ':
            ...
            self.syringeIndex = str(self.syringeBDPDiameterLookup[syringeVolume])+" "
        elif syringe_type == 'bdg ':
            self.syringeIndex = str(self.syringeBDGDiameterLookup[syringeVolume])+" "
        else:
...
class Pump(object):
    ...
    def __init__(self, name, USBAddress,syringe1, syringe2=None):
        ...
        self.amountUsed = 0  # This is used to keep track of solution consumption.
        # Connect to the pump, unless testMode is enabled.
        self.pump = serial.Serial(port=self.USBAddress, baudrate=115200,stopbits = 2, bytesize = 8)
        # Initialize pump settings
        self.writePort('nvram none\r')  # Turn off non-volatile RAM to increase pump CPU process speed and protect the pumps.
        if syringe2 is not None:
            self.writePort('force ' + 'ab ' + '100\r')  # Set to the pump's maximum force to ensure they  don't stall.
        else:
            c = self.syringeA.identifier
            self.writePort('force ' + self.syringeA.identifier + '100\r')  # Set to the pump's maximum force to ensure they  don't stall.
    ...
    def inject(self, syringe,flowRate, time,unit):
        ...
        if flowRate != 0:  # Guard against any flow rate of 0, as that cannot be set in the pumps.
            self.writePort('ctime ' + identifier + '\r')  # Clear any target time.
            self.writePort('cvolume ' + identifier + '\r')  # Clear any target volume.
            self.writePort('tvolume ' + identifier + str(abs(flowRate) * time / 60) + 'ml\r')
            if flowRate>0:
                ...
                self.writePort('irate ' + identifier + str(flowRate) + ' '+unit+'\r')
                # Set the flow rate.
                self.writePort('irun ' + identifier + '\r')  # Start the injection.
                self.amountUsed += flowRate * time / 60  # Add to the amount of solution used.
                print('Injecting ' + self.name + ' at ' + str(format(flowRate, '.2f')) + unit+' for ' + str(time) + 's.') #Print to the console that the syringe injection information.
            else:
                self.writePort('wrate ' + identifier + str(abs(flowRate)) +' '+unit+'\r')
                # Set the flow rate.
                self.writePort('wrun ' + identifier + '\r')  # Start the injection.
                self.amountUsed += flowRate * time / 60  # Add to the amount of solution used.
...
if __name__ == "__main__":
    ...
    fl = 10
    t = 60
    test.inject(syringeA,fl,t,"ul/min")
