...
def process_sls_1500(total_seconds, file_name):
    ...
    # Create a serial port object for communication on port 'COM3' with baudrate 115200
    port = ShdlcSerialPort(port='COM3', baudrate=115200)
    # Create a flow meter device using the ShdlcConnection and the port,
...
class Set_Linearization(ShdlcCommand):
    def __init__(self, bool):
        super(Set_Linearization, self).__init__(
            ...
            # False: Raw Measurement, True: Linearized Measurement
            data=pack("B", int(bool)),
            max_response_time=0.2,  # Maximum response time in Seconds
        )
...
def decode_unit(bitcode):
    ...
    unit_string = dim_dict[dim_unit_code] + \
        vol_dict[vol_unit_code] + time_dict[time_unit_code]
    return unit_string
...
class SLS_1500Device(ShdlcDeviceBase):
    ...
    def Get_Version(self):
        ...
        firmware_Major, firmware_Minor, firmware_debug, hardware_Major, hardware_Minor, shdl_major, shdl_minor = unpack(
            '>BBBBBBB', raw_response)
    ...
    def Get_Measurement_Buffer(self, plot=False):
        ...
        byte_chunks = [raw_response[i:i+2]
    ...
    def Continuous_Measure_and_Save(self, duration_s, filename, set_flow_rate_string=None):
        ...
        # 16 bit resolution, calib field 0, linearization on
        self.Sensor_Command_Settings(
            resolution=b"\x10", calib_field=b"\x00", set_linearization=True)

        # Duration divided by buffer fill duration (10ms)
        retrievals = np.ceil(duration_s / MEASURING_INTERVAL)
        ...
        df = pd.DataFrame(columns=['ms', 'mL/min'])
        time_array = None
        q_array = None
        i = 0
        ...
        while i < retrievals:
            ...
            if i == 0:
                ...
                time_step = np.linspace(
                    last_s_value, last_s_value+elapsed_time_s, 100)
                time_array = np.hstack((time_array, time_step))
                q_array_step = np.array(
                    self.Get_Measurement_Buffer())/SCALE_FACTOR
        ...
        np.savetxt(filename, np.c_[
                   time_array, q_array], delimiter=',', header='s,mL/min', comments='', fmt='%f')

# Error version of continuous measurment
