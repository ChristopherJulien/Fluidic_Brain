...
def Pressure_Controller(nb_controllers: int=1):
    ...
    if nb_controllers == 2:
        error_code, _p_min_1, _p_max_1 = fgt_get_pressureRange(1, get_error=True)
        if error_code != fgt_ERROR.OK:
            raise SystemExit("Failed to find the second controller pressure range")
...
​
        ...
        assert p_min_0.get() <= start_p1 and p_min_1.get() <= start_p2, "Start pressure must be greater than minimum pressure"
        assert p_min_0.get() <= end_p1 and p_max_1.get() <= end_p2, "End pressure must be greater than minimum pressure"
        assert start_p1 <= p_max_0.get() and start_p2 <= p_max_1.get(), "Start pressure must be lower than maximum pressure"
        assert end_p1 <= p_max_0.get() and end_p2 <= p_max_1.get(), "End pressure must be lower than maximum pressure"
        assert self.nb_controllers == 2, "Should be two controllers"
        assert nb_big_ramp_controller in [0, 1], " The controller for the big ramp should be 0 or 1"
        ...
        for p_0 in [p_range1, p_range2][nb_big_ramp_controller]:
                ...
                for p_1 in [p_range1, p_range2][nb_many_ramps_controller]:
                    ...
                    if nb_big_ramp_controller == 0:
                        print(f'Set pressure to p_0 = {p_0:.1f} mbar and p_1 = {p_1:.1f} mbar', end="\r")
                        self.inputs_list.append([p_0, p_1])
                    else:
                        print(f'Set pressure to p_0 = {p_1:.1f} mbar and p_1 = {p_0:.1f} mbar', end="\r")
