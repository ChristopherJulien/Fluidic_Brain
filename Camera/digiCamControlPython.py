...
class Camera:
    ...
    def runCmd(self, cmd: str):
        ...
        r = subprocess.check_output(
            "cd %s && CameraControlRemoteCmd.exe /c %s" % (self.exeDir, cmd), shell=True).decode()
    ...
    def __setCmd(self, cmd: str, value: str):
        ...
        r = subprocess.check_output("cd %s && CameraControlRemoteCmd.exe /c set %s" %
                                    (self.exeDir, cmd + " " + value), shell=True).decode()
        if 'null' in r:  # Success
    ...
    def __getCmd(self, cmd: str):
        ...
        r = subprocess.check_output(
            "cd %s && CameraControlRemoteCmd.exe /c get %s" % (self.exeDir, cmd), shell=True).decode()
    ...
    def __listCmd(self, cmd: str):
        ...
        r = subprocess.check_output(
            "cd %s && CameraControlRemoteCmd.exe /c list %s" % (self.exeDir, cmd), shell=True).decode()
        if 'Unknown parameter' in r:  # Error
            print("Error: %s" % r[109:])  # Format output message
            return -1
        else:           # Success
            ...
            print("List of all possible %ss: %s" %
                  (cmd, returnList))  # Format output message
            return returnList
